...
In the example above, by default +bar.rb+ will be executed before +foo.rb+, resulting in a constant lookup error, because the +Foo+ isn't defined yet when we execute +bar.rb+.
